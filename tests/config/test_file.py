...
class TestConfigFile(unittest.TestCase):
    ...
    def test_lines(self):
        """ Test that the lines are correctly intialised """

        fn = file.File("/path/to/config")

        self.assertEqual(fn.lines, [], "by default, lines are empty")
    ...
    def test_descriptions(self, os_path_expanduser: unittest.mock.Mock):
        ...
        for (actual, intended) in zip(fn.descriptions, results):
    ...
    def test_locals(self, os_path_expanduser: unittest.mock.Mock):
        ...
        # check that the yielding works properly
        for (actual, intended) in zip(fn.locals, results):
